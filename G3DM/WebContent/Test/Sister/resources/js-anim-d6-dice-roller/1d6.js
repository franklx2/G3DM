...
D6.dice = function(numDice, callback, callbackData, useImages, buttonLabel) {
	...
	if (buttonLabel != "none") {
		genHtml += "<div id='diceform'><form><input class='btn btn-info' type='button' id='dicebutton' value='" + buttonLabel + "' onclick='D6AnimBuilder.get(\"dice\").reset(); D6AnimBuilder.get(\"dice\").start()' /></form></div>";
	}
	genHtml += "</div>";
	D6.genHtml = genHtml;
...
D6.middleManCallback = function(middleManData) {
	...
	for(var i = 0; i < array.length; i++)
	{
		...
		{
			topThree[target] = array[i]; 
		}
		target++;
	}
	for (i=0; i<3; ++i) {
		resultsTotal += topThree[i];
	}
	for(i=0; i <3; i++)
	{
		console.log(i + "th roll: " + topThree[i]);
	}
